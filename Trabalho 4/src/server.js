...
io.on('connection', (socket) => {
    ...
    socket.on('message', (data) => {
        ...
        if (room && room.has(socket.id)) {
            ...
            // O usuário não está conectado à sala
    ...
    socket.on('leave-room', (roomId, userId) => {
        socket.leave(roomId);  // O usuário sai da sala
        console.log(`Usuário ${userId} saiu da sala: ${roomId}`);
        io.to(roomId).emit('user-disconnected', userId);  // Notifica outros usuários na sala
    });

    socket.on('disconnect', () => {
        console.log('Usuário desconectado', socket.id);
        io.emit('user-disconnected', socket.id);  // Notifica sobre a desconexão do usuário
    });
});
